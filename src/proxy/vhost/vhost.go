...
func (p *VHostProxy) Start() error {
	...
	// zerolog instead of the default ErrorLog, which writes to stderr
	// unstructured (`http: TLS handshake error from ...`). These fire
	// constantly on internet-exposed bastions, so the writer logs them at
	// debug and surfaces everything else as a warning.
	p.s.Server.ErrorLog = stdlog.New(
		&httpErrorLogWriter{port: p.port},
		"", 0,
	)
